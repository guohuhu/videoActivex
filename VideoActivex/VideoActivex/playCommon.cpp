...
DWORD WINAPI rtspProc()
{
	TaskScheduler* scheduler = BasicTaskScheduler::createNew();
	env = BasicUsageEnvironment::createNew(*scheduler);
	h264Decoder1 = new H264Decode();
...
int recvRTSP(HDC hdc1,LPCTSTR rtsp_URL) {
...
//	char **argv = NULL;
 // progName = argv[0];
  ...
  //streamURL = argv[1];
	//streamURL = "rtsp://192.168.3.105:8554/webcam";
  // Create our client object:
	//streamURL = (char *)malloc(strlen((LPSTR)rtsp_URL));
	//streamURL = (char const *)rtsp_URL;
	//memcpy(streamURL,(LPSTR)rtsp_URL,strlen((LPSTR)rtsp_URL) );
//	streamURL + strlen((LPSTR)rtsp_URL) = '\0';
	...
	MessageBox(NULL,_T("OK "),_T("Test "),FALSE);
   //MessageBox(NULL,(LPCTSTR)streamURL,_T("Test "),FALSE);
	hDC = hdc1;
	DWORD Id;
	HANDLE hThread = CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)rtspProc,NULL,0,&Id);
...
//	H264IPCFileSink *fileSink;
//	fileSink->pdc_gloab = pdc_gloab1;
	FileSink* fileSink;
...
//	fileSink->h264Decode = h264Decoder;
	fileSink->h264Decode = h264Decoder1;
	fileSink->h264Decode->init_h264_decode();
//	fileSink->h264Decode->hDC = hdc1;
